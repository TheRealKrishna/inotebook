...
const NoteState = (props)=>{
  ...
  const fetchNotes = async()=>{
    let notesInitial = await fetch(`${process.env.REACT_APP_API_URL}/notes/fetchnotes`, {
      method: "POST",
      headers: {
    ...
    setNotes(notesInitial.notes)
      ...
      const addNote = async (note)=>{
        toast.promise(new Promise(async(resolve, reject)=>{
          await fetch(`${process.env.REACT_APP_API_URL}/notes/addnote`, {
            method: "POST",
            headers: {
              'auth-token': localStorage.getItem("auth-token"),
      ...
      const deleteNote = async (noteId)=>{
        toast.promise(new Promise(async(resolve, reject)=>{
          await fetch(`${process.env.REACT_APP_API_URL}/notes/deletenote/${noteId}`, {
          method: "PUT",
          headers: {
            'auth-token': localStorage.getItem("auth-token"),
          },
      ...
      const editNote = async (noteId, note)=>{
       toast.promise(new Promise(async (resolve, reject) => {
          await fetch(`${process.env.REACT_APP_API_URL}/notes/updatenote/${noteId}`, {
            method: "PUT",
            headers: {
              'auth-token': localStorage.getItem("auth-token"),
              'Content-Type': 'application/json',
